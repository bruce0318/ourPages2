...
export async function statProvince(map) {
    ...
    const res_points = await fetch('http://47.111.136.83:5500/api/statProvinceForCui', {
        method: 'POST',
    ...
    try{
        ...
        if (!pointsData.features || pointsData.features.length === 0) {
            ...
            content += `<div><p class="location-title">崔泽铭走过${input}的${uniqueCity.size}个城市：</p></div>`;  // 个人足迹地图
